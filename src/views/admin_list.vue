  ...
  <div>
    <v-row>
      ...
      <v-col cols="9">
        ...
        <v-data-table :headers="headers" :items="adminList" :items-per-page="5" class="elevation-3">
          <!-- Buttons -->
          <template v-slot:item.actions="{ item }">
            <!-- Demote User -->
            <v-btn class="mr-2" color="primary" fab x-small elevation="2" @click="onClickDemoteUser(item)"
              :disabled="disableBtn">
    ...
    <v-dialog v-model="DemoteDialog" width="680">
      <v-card>
        ...
        <v-card-actions class="justify-end">
          <v-btn color="#ffd343" class="black--text" dark @click="DemoteUser()">Demote</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
